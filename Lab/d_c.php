...
VALUES (
NULL , '$ya[RegId]' , 'LR:$ya[id]' , '$ya[NamaPasien]' , '$Pembayar' , '$hari' , '$TglBayar' , 
'$BlnBayar' , '$ThnBayar' , '$JamBayar' , '$a[Nama]' , '' ,'$TypeBayar', '$jop[NamaBank]', '$Tunai', '$Kk' , '$Db' , '$JamE', '$Asli' , '$Asli', '$Persen', '$KodePer', '$NamaPer', '$TxnJalan', '$Farmasi', '$Laborat', '$Stat', '$crg', 'LAR', 'Laboratorium Luar', '$Keterangan', 'LR:$ya[id]'
)");
$kjdfd_Dfdfd = mysql_query("SELECT id FROM kwitansirj WHERE RegId='$ya[RegId]'");
$xo = mysql_fetch_array($kjdfd_Dfdfd);
 if($simp){
 $upda = mysql_query("UPDATE txnrad SET Stat=1 WHERE id='$id'");
 }

 header("Location:./s.php?id=$xo[id]");
}
?>