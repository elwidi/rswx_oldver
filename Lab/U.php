...
<span class="style1">HASIL LAB TERKAIT 
<?
if($dimas=='ela'){
$KSKJS = mysql_query("SELECT COUNT(id) as nn FROM cetak_hasil_labinap where IdAsal='$IdDetil'");
}else{
$KSKJS = mysql_query("SELECT COUNT(id) as nn FROM cetak_hasil_lab where IdAsal='$IdDetil'");
}
$ye = mysql_fetch_array($KSKJS);
echo $ye[nn];
?> 
.</span></div></td>
</tr>
<tr>
  <td colspan="3">
	...
	<a href="d4_f.php?NoBukti=<? echo $NoBukti; ?>&dimas=<? echo $dimas; ?>&id=<? echo $id; ?>&IdDetil=<? echo $IdDetil; ?>"   onMouseOver="movein(this,'');window.status='';" onMouseOut="moveout(this);window.status='';" style="text-decoration:none">YA</a> || 
