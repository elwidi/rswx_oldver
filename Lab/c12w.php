...
echo $ya_p[NamaDokter];	
    ...
    <tr  ><TD height="34" colspan="3"><div align="center">
	  ...
	  <a href="c12w.php?awal=<? echo $awal+23; ?>&id=<? echo $id; ?>" onMouseOver="movein(this,'');window.status='';" onMouseOut="moveout(this);window.status='';"   style="text-decoration:none">HASIL PEMERIKSAAN LABORATORIUM<br> 
        RAWAT JALAN</a>
...
?><TR  >
 ...
 <?
 $no=1;
 $kjdfdfD_Dfdfd = mysql_query("SELECT * FROM tampil_lab WHERE IdPasien='$id' ORDER BY id limit $awal,23");
 while($nn = mysql_fetch_array($kjdfdfD_Dfdfd)){
  $paw = $no%2;
  if($paw==1){
  $warna=' bgcolor="#D7FDB0"';
  }else{
  $warna='';
  }
   if($nn[Level]==1){
   ?><tr <? echo $warna; ?>   class="style8">
   <td><div align="left">
   <?
   echo $nn[Kelompok2];
   ?>
   </div></td>
      <td><div align="center"><? echo $nn[Hasil]; ?></div></td>
   <td><div align="center"><? echo $nn[Satuan]; ?></div></td>
   <td><div align="center"><? echo $nn[Rujukan]; ?></div></td>
   <td>&nbsp;</td>
   </tr><?
   }
   ?><?
   if($nn[Level]==2){
   ?><tr <? echo $warna; ?> class="style3">
