...
//end count date
	    $isikan = mysql_query("INSERT INTO `txnrad` (`id`, `RegId`, `TglLahir`, 
 		`BlnLahir`, `ThnLahir`,`JenisKelamin`,`Tgl`, `Bln`, `Thn`, `Usia`,`LbhBulan`,
		 `NamaPasien`, `KodeDokter`, `NamaDokter`, `KdTgJawab`, `NamaTgJawab`, `KdRujukan`, `NamaPerujuk`,`HpTelp`, `Alamat`, `Opr`, `Stat`, `NoPasien`)
		  ...
		  '$xz2[KodeDokter]','$xz2[NamaDokter]','$xz3[KodeDokter]','$xz3[NamaDokter]','$HpTelp', '$Alamat','$a[Nama]','0','$NoPasien')");
		if($isikan){
		$Bus = mysql_query("SELECT id FROM txnrad WHERE RegId='$mbah'");
		$x = mysql_fetch_array($Bus);
		header("Location:./nasi_kucing.php?hari=$x[id]");
	}else{
	echo "SIMPAN DATA GAGAL,....<br>INSERT INTO `txnrad` (`id`, `RegId`, `TglLahir`, 
 		`BlnLahir`, `ThnLahir`,`JenisKelamin`,`Tgl`, `Bln`, `Thn`, `Usia`,`LbhBulan`,
		 `NamaPasien`, `KodeDokter`, `NamaDokter`, `KdTgJawab`, `NamaTgJawab`, `KdRujukan`, `NamaPerujuk`,`HpTelp`, `Alamat`, `Opr`, `Stat`, `NoPasien`)
		  VALUES (Null, '$mbah', '$TglLahir', '$BlnLahir', '$ThnLahir','$JenisKelamin',
 		  '$tgl', '$bln', '$thn', '$tahunipun','$sasinipun-$dintenipun', '$NamaPasien','$xz3[KodeDokter]','$xz3[NamaDokter]',
