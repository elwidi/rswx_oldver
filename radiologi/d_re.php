...
-->
 ...
 <table border="1">
 <tr>
 <td width="530" colspan="3"><div align="left" class="style1">
 HALAMAN PEMBAYARAN TRANSAKSI <BR />
 PEMERIKSAAN RADIOLOGI LUAR
 </div></td>
 </tr>
 <tr>
 ...
 <td>:</td>
   <td><div align="left"><?
$kdjkdjkfd= mysql_query("SELECT SUM(Harga) AS hh FROM txnjalan WHERE RegId='$ya[RegId]'");
$up = mysql_fetch_array($kdjkdjkfd);
echo number_format($up[hh]);   
   ?>
   <input type="hidden" name="Asli" value="<? echo $up[hh]; ?>">
    || 
   <a href="#"  style="text-decoration:none" onClick="wCompare = window.open('ivr.php?id=<? echo $id; ?>', 
  'wCompare', 'width=580,height=240,left=470,Top=129,resizable=yes,directories=no,status=no,toolbar=no,menubar=0,location=no,scrollbars=yes'); 
 ...
 <textarea rows="3" cols="50" name="Keterangan" class="style2">PELUNASAN PEMBAYAR RADIOLOGI LUAR AN:<? echo $ya[NamaPasien]; ?></textarea>
 </div></td>
 </tr>
 <tr>
   <td>TYPE BAYAR </td>
