...
<?
$juy = 1;
$ddt = mysql_query("SELECT * FROM jualobat_tmp WHERE JamTran='$NoResep' AND Qty>0 AND apa_r!=1");
...
<? $juy++; } 
$jkjioi = mysql_query("SELECT COUNT(id) AS jh FROM jualobat_tmp WHERE JamTran='$NoResep' AND apa_r=1");
...
?></em></div></td><td><div align="right" class="style11"><em>
  <? 
$jauiu = mysql_query("SELECT SUM(JmlHargaBeli) AS jh FROM jualobat_tmp WHERE JamTran='$NoResep' AND apa_r=1 AND Qty>0");
...
$ggh = mysql_query("SELECT SUM(JmlHargaBeli) AS jh FROM jualobat_tmp WHERE RegId='$ii[RegId]' AND Qty<0");
