...
$Thn=$Thn;
  ...
  //
  $num=1;
   $kdidkdidk_99d0 = mysql_query("SELECT RegId,id,NoPasien,NamaPasien,Qty,NamaObat,JamTran,HargaBeli,KodeObat FROM jualobat WHERE KodeObat='$ff[IdObat]'");
...
$noma=1;
$tar =1;
$JKJ = mysql_query("SELECT IdAsal,IdPo77,IdBarang,Qty,id,NamaBarang,Pengali,Harga,Total,NamaSatuan,IdFaktur,IdPox FROM podetil77x WHERE IdBarang='$ff[IdObat]'  
 AND  Stat=2 ORDER BY id asc");
...
while($awalX<=$Tot){
 $wa = $awalX%2;
 $pak = explode(".",$RegId[$tar]);
 if($pak[0]=='R'){
 $class='class="style289"';
 }else{
 $class='class="style222"';
 }
if($wa==1){
$warna2='  bgcolor="#C9FEBC"';
}else{
$warna2=' bgcolor="#EFFFD2"';
}
?>
 <tr <? echo $warna2; echo $class; ?>   >
...
if($vx[NoPo]==''){
 echo $kk[IdPo77];
//
$jujsj = mysql_query("SELECT Harga FROM podetil77x WHERE Kait='$kk[IdPo77]' AND IdPo77 not like 'R%'");
$pa = mysql_fetch_array($jujsj);
//
$jujsj_yyyyyy = mysql_query("update podetil77x Set Harga='$pa[Harga]' WHERE id='$kk[id]'");
...
?></div></td>
<td width="55"><div align="right"><? echo number_format($kk[Harga],2);
$tot = $tot + $kk[Harga];
 ?></div></td>
 <?
 ...
 <td width="183"><div align="left"><?
   echo ucfirst(strtolower($Pasien[$tar]));
 ?></div></td>
 <td><div align="left"><?
 if($Id[$tar]>0){
 $Jual =($kk[Harga]);
...
$KeyID="$ff[KeyID]";
 ...
 ?></div></td><td width="17"><div align="center"><? $Kait="R.$Id[$tar]";
 echo $Kait; 
 //
  $oaiao=mysql_query("UPDATE podetil77x SET Kait='$Kait' WHERE id='$kk[id]'");
   ?></div></td>
  <?  $awalX2++;}else{ 
...
?> <tr>
   <td colspan="2" bgcolor="#CEFDC8"><div align="center"><em>Total</em></div></td>
    ...
    <td colspan="6" bgcolor="#CEFDC8"><em>Pakai : <? 
	...
	//$Pakai = $Pakai -$bpa[hh]; 
	echo number_format($Pakai,2);
   	  $tot_3 = $tot_3 + $Pakai; ?>  
	 <?
	
if($bpa[hh]>0){
	?><b>   Retur :    ( <?  echo number_format($bpa[hh]);  
	       ?> ) </b> 
	<? 
	} 	
 $Kunci =$ff[KeyID]; 
...
$noma++; } ?>
 <tr>
 <td colspan="7" bgcolor="#3366FF"><span class="style4">GT 1 : Persedian : <span class="style5"><? echo $NamaBarang; ?>, </span>Tot  <span class="style5"><? echo number_format($tot_1,2);
 
  ?>. </span>Tot Perse :   <span class="style5"><? echo number_format($tot_2,2); 
 ?>. </span>Tot Pakai (Hpp) <span class="style5">:    
       <?  
//	   $tot_3 = tot_3-$bpa[hh];
	   echo number_format($tot_3,2);
...
<tr>
  ...
  echo number_format($tra); 
   ...
   ?> <font color="#009933"> || Ppn : <? echo number_format($tot_Ppn);  ?></font>
  </strong></div></td>
</tr>
</table>
</div>
