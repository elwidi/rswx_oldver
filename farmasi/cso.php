 ...
 <script type='text/javascript' src='js/jquery-1.3.2.min.js'></script>
  ...
  <script language="javascript">
    $(function(){
		...
		$('#suggest2').autocomplete('ss_0.php', 
			{
				...
				parse: function(data){ 
					...
					for (var i=0; i < data.length; i++) {
						parsed[i] = {
							data: data[i],
							value: data[i].NoFaktur // nama field yang dicari
...
str += '<u>'+data.NoFaktur+'</u><br />SO '+data.NoSo+'<br />TGL: '+data.TglDtgl+' / '+data.BlnDtg+' / '+data.ThnDtg;;					
					str += '';
					return str;
				},
...
var Result =''+data.id+'';;
location.href="minum_kopi_makan_gorengan.php?id=" + Result;
			}
		);
	});
  </script></head>
