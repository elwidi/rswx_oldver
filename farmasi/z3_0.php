...
<body>
<table border="0"   onClick="location.href='../Daftar/fito_sayangku.php';" onMouseOver="movein(this,'');window.status='';" onMouseOut="moveout(this);window.status='';">
<tr class="style3">
...
<td><div align="center"><span class="style14">:</span></div></td>
<td><div align="left" class=" "><? echo $ii[KetDokter]; ?></div></td>
</tr>
<tr class="style3">
<td colspan="3">&nbsp;</td>
</tr>
...
<?
$juy = 1;
$ddt = mysql_query("SELECT * FROM jualobat2 WHERE JamTran='$NoResep' AND Qty>0 AND apa_r!=1");
...
<? $juy++; } 
$jkjioi = mysql_query("SELECT COUNT(id) AS jh FROM jualobat2 WHERE JamTran='$NoResep' AND apa_r=1");
...
<td class="style12"><em><?
$IKJKD  = mysql_query("SELECT Nama FROM racik WHERE NoResep='$NoResep'");
$yh = mysql_fetch_array($IKJKD);
$pak = strlen($yh[Nama]);
if($pak>0){
echo strtoupper($yh[Nama]); 
}else{
?>RACIKAN<? } ?></em></td><td><div align="center" class="style11"><em><?
...
?></em></div></td><td><div align="right" class="style11"><em>
  <? 
$jauiu = mysql_query("SELECT SUM(JmlHargaBeli) AS jh FROM jualobat2 WHERE JamTran='$NoResep' AND apa_r=1 AND Qty>0");
$uk = mysql_fetch_array($jauiu);
$JHGJHG = mysql_query("SELECT Racikan FROM  racikembal  WHERE id=1");
$gu = mysql_fetch_array($JHGJHG);
$jkjoip = $uk[jh]+($gu[Racikan]);
...
$ggh = mysql_query("SELECT SUM(JmlHargaBeli) AS jh FROM jualobat2 WHERE RegId='$ii[MstKey]' AND Qty<0");
