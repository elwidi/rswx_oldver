...
<body>
<table border="0">
<tr   onClick="location.href='../Daftar/fito_sayangku.php?ade=3';" onMouseOver="movein(this,'');window.status='';" onMouseOut="moveout(this);window.status='';" class="style3">
...
<?
$juy = 1; $nom=1;
$ddt = mysql_query("SELECT * FROM jualobat2 WHERE JamTran='$NoResep' AND Qty>0 AND apa_r=1 AND stat=1 ORDER BY apa_r DESC");
while($ss = mysql_fetch_array($ddt)){
?>
<tr bordercolor="#000000" class="style3" valign="top">
<td><div align="center" class="style8"><?  
if($nom==1){
echo $juy; 
} ?></div></td>
<td><div align="left" class="style8"><? if($ss[apa_r]==1){ 
echo "<i>"; echo strtoupper($ss[NamaObat]); echo " (R-$ss[stat])</i>";
...
<? }  
$juyX = 2; $nomX=1;
$ddtx = mysql_query("SELECT * FROM jualobat2 WHERE JamTran='$NoResep' AND Qty>0 AND apa_r=1 AND stat=2 ORDER BY apa_r DESC");
...
echo "<i>"; echo strtoupper($ssx[NamaObat]); echo " (R-$ssx[stat])</i>";
...
$sayangx = $sayangx + $ssx[JmlHargaBeli];
 ?></div></td>
</tr>
...
$juyZ = 3; $nomZ=1;
$ddtz = mysql_query("SELECT * FROM jualobat2 WHERE JamTran='$NoResep' AND Qty>0 AND apa_r=1 AND stat=3 ORDER BY apa_r DESC");
...
echo "<i>"; echo strtoupper($ssz[NamaObat]); echo " (R-$ssz[stat])</i>";
...
$sayangz = $sayangz + $ssz[JmlHargaBeli];
 ?></div></td>
</tr>
...
$juyZ2 = 4; $nomZ2=1;
$ddtz2 = mysql_query("SELECT * FROM jualobat2 WHERE JamTran='$NoResep' AND Qty>0 AND apa_r=1 AND stat=4 ORDER BY apa_r DESC");
...
echo "<i>"; echo strtoupper($ssz2[NamaObat]); echo " (R-$ssz2[stat])</i>";
...
$sayangz2 = $sayangz2 + $ssz2[JmlHargaBeli];
 ?></div></td>
</tr>
...
<? } 
$juyZ3 = 5; $nomZ3=1;
$ddtz3 = mysql_query("SELECT * FROM jualobat2 WHERE JamTran='$NoResep' AND Qty>0 AND apa_r=1 AND stat=5 ORDER BY apa_r DESC");
...
echo "<i>"; echo strtoupper($ssz3[NamaObat]); echo " (R-$ssz3[stat])</i>";
...
$sayangz3 = $sayangz3 + $ssz3[JmlHargaBeli];
?></div></td>
</tr>
...
<? }
$ujdujduddf =mysql_query("SELECT COUNT(id) AS jj FROM jualobat2 WHERE JamTran='$NoResep' AND Qty>0 AND apa_r=1");
...
<? } ?>
 <?
 $juy2=1;
$ddt2 = mysql_query("SELECT * FROM jualobat2 WHERE JamTran='$NoResep' AND Qty>0 AND apa_r!=1 ORDER BY apa_r DESC");
...
echo "<i>"; echo strtoupper($ss2[NamaObat]); echo " (R-$ss2[stat])</i>";
