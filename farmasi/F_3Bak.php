...
<?
$nomer =1;	$bas=1;
$kdjd = mysql_query("SELECT * FROM rj_resep WHERE IdPasien='$id' ORDER BY NoResep");
while($mm = mysql_fetch_array($kdjd)){
 $no=1; 
	$namjsm = mysql_query("SELECT * FROM jualobat_tmp WHERE RegId='$ii[RegId]' AND JamTran='$mm[NoResep]' ORDER BY apa_r,JamTran DESC");
	while($aa =  mysql_fetch_array($namjsm)){
	if($NoResep!=$mm[NoResep] && $NoResep!='' || $Rck!=$aa[apa_r] && $Rck!=''){
	?><tr class="style1">
...
//$ksiski_Dfdf = mysql_query("SELECT COUNT(id) AS jj FROM jualobat_tmp WHERE ");
