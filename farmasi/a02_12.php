...
include "../alamat.php";
//
header("Location:./dimas_mau_tk.php?dimas=$dimas&id=$id");
...
if($anak=='dimas' && $hp_01mb[id]){
	...
	$nomer=$awal;
	if($dimas>0){
	$kjdk  = mysql_query("SELECT * FROM podetil77x WHERE IdPox='$id' and IdFaktur='$dimas'");// LIMIT $awal,7");
 	}else{
	$kjdk  = mysql_query("SELECT * FROM podetil77x WHERE IdPox='$id' "); //LIMIT $awal,7");
...
<td><div align="center">
	...
	<?
		if($hh[Stat]==2){
	$TglE=date("d");$BlnE=date("m"); $ThnE=date("Y"); $JamE=date("H:i:s");
	 
	$awalane =1;
	while($awalane<=$hh[Qty]){
	$NoUnik="FA.$uh2[id]-$hh[id].$awalane";
	$KodeSumber="PO:$id.$hh[Stat]-$hh[id]";
	$isis = mysql_query("INSERT INTO  `hpp` (`id`, `KodeSumber`, `IdObat`, `NamaObat`, `Harga`, `Stat`, `Tgl`, `Bln`, `Thn`, `Jam`, `NoUnik`) VALUES
	 (NULL, '$KodeSumber', '$hh[IdBarang]', '$hh[NamaBarang]', '$hh[Harga]', '0', '$TglE', '$BlnE', '$ThnE', '$JamE','$NoUnik')");
...
if($yh[Diskon]>0){
	...
	$toto=$toto;
 	 //
	 	 $kdjfkjdfd_Dfdfd1034= mysql_query("SELECT COUNT(id) AS hh FROM podetil77x WHERE IdPo77='$id' AND Kait=2");
