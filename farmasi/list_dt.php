...
// tambahan jasa onMouseover
 ...
 <div align="left">
   <table width="1071" border="0" cellpadding="1" cellspacing="2" class="style1">
    ...
    <?
	   if($a[Level]==7){ ?>
	   <td bgcolor="#ECE9D8">INTEGRASI</td>
    <td bgcolor="#FFFFFF">NAMA AKUNT</td>
	<td bgcolor="#FFFFFF">TGLJAM</td>
	<? }else{
     ...
     <td width="109" bgcolor="#FFFFFF"><span class="style8">TGLJAM</span></td>
     <? } ?></tr><?
   $NO=1;
    if($iin==2){
...
//   $upa = mysql_query("UPDATE dataobat SET HargaJual='$c[HargaBeli]' WHERE id='$c[id]'");
  ...
  'wCompare', 'width=600,height=500,left=470,Top=80,resizable=yes,directories=no,status=no,toolbar=no,menubar=0,location=no,scrollbars=yes'); 
    wCompare.focus();"  onMouseOver="movein(this,'');window.status='';" onMouseOut="moveout(this);window.status='';" >
     ...
     <td><div align="left"><? echo $c[KodeObat];
	    $kdfjdfd_dfdfdf = mysql_query("SELECT COUNT(id) as kk FROM isipaketdetil WHERE IdPaket='$c[id]'");
		 ...
		 if($hv[kk]>0){ ?> ( <font color="#990000"><i><?  echo $hv[kk]; ?></i></font> }<? } ?></div></td>
     <td><div align="left"><? 
	 echo ucfirst(strtolower($c[NamaObat])); ?></div></td>
	  <td><div align="left"><?
	  if($c[IdGol]==778 || $c[IdGol]==779){
	 ?><font color="#990000"><? echo ucfirst(strtolower($c[NamaGolongan])); ?></font><?  
	  }else{
	 echo ucfirst(strtolower($c[NamaGolongan]));
	}  ?></div></td> 
       <td bgcolor="#ECE9D8"><div align="right"><? echo number_format($c[Hna]); ?></div></td>
	 <?
	if($a[Level]==7){ 
	...
	<td><div align="left"><? echo "$hp[TglJam]"; ?></div></td>
	<? }else{
	?>   <td><div align="left"><? echo ucfirst(strtolower($c[Satuan1])); ?></div></td>
	     <td><div align="left"><? echo ucfirst(strtolower($c[Satuan2])); ?></div></td>
	  <td><div align="center"><?
	  include "huting.php";
	  ?></div></td>  
	...
	?>   <td <? 	   if($nh[id]){ ?> bgcolor="#FF99CC"<? } ?>><div align="left"><? 
 	 echo strtoupper($c[Opr]);  
	   ?></div></td>
	   <td><div align="left"><? echo $c[TglJam]; ?></div></td>
	   <? } ?>
   </tr>
