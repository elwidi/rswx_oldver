...
<head><script type='text/javascript' src='js/jquery-1.3.2.min.js'></script>
  ...
  <script language="javascript">
    $(function(){
		...
		$('#suggest3').autocomplete('loka_s3.php', 
			{
				...
				parse: function(data){ 
					...
					for (var i=0; i < data.length; i++) {
						parsed[i] = {
							data: data[i],
							value: data[i].NamaKot  // nama field yang dicari
...
var Result =''+data.id+'';;
location.href="x2qaa.php?IdKel=<? echo $IdKel; ?>&IdKec=<? echo $IdKec; ?>&hari=mitra&IdKot=" + Result;
			}
		);
	});
  </script></head>
