...
<div align="left">
  <table width="1365" height="308" border="0" cellpadding="2" cellspacing="2">
    <tr valign="top">
...
LIST HALAMAN PASIEN RAWAT INAP
   ...
   <td colspan="12" valign="top">
	...
	<tr>
	 ...
	 <? } ?>
	  <input type="button" name="dfdfdf" value="List Kaber" onClick="location.href='../Kasir/fito_sayangku2.php';"  class="style6" style="background:#FF9933"  >
   ...
   <td width="130" rowspan="2" bgcolor="#666666"> <span class="style17">PENJAMIN</span></td>
	  ...
	  >
	  <td width="133" rowspan="2" bgcolor="#666666"> <span class="style17">RUJUKAN</span></td
	  ></tr>
	   <tr bgcolor="#FAEE0E" >
...
$warna3='bgcolor="#FFFF00" valign="top"';
 }else{
$warna=' valign="top" ';
$warna3='bgcolor="#CEFCBF"  valign="top"';
}
...
}elseif($a[Level]==91 || $a[Level]==9  || $a[Level]==212){
...
<? 
 ...
 <?
	  ...
	  <tr   <? echo $warna;?> onClick="wCompare = window.open('txn/h2.php?id=<? echo $nn[id]; ?>', 
  'wCompare', 'width=920,height=130,left=270,Top=210,resizable=yes,directories=no,status=no,toolbar=no,menubar=0,location=no,scrollbars=yes'); 
...
echo "($jm[hh])";	  ?></i></font><? } 
  ...
  $jsuj_s3x= mysql_query("SELECT Diagnosa,KodeIcd FROM icd_ri WHERE IdPasien='$nn[id]' AND Urutan<=1");
	  while($bo3x = mysql_fetch_array($jsuj_s3x)){
echo strtoupper($bo3x[Diagnosa]); echo ",";
}    ?></div></td>
	  <td><div align="left"><?
	  $jsuj_s3= mysql_query("SELECT Diagnosa,KodeIcd FROM icd_ri WHERE IdPasien='$nn[id]' AND Urutan=2");
	  while($bo3 = mysql_fetch_array($jsuj_s3)){
echo strtoupper($bo3[Diagnosa]); echo ",";
}  
	  ?></div></td>
	  <td><div align="left"><?
	  $jsuj_s3= mysql_query("SELECT Diagnosa,KodeIcd FROM icd_ri WHERE IdPasien='$nn[id]' AND id!='$bo[id]' AND Urutan>=3");
	  while($bo3 = mysql_fetch_array($jsuj_s3)){
echo strtoupper($bo3[Diagnosa]); echo ",";
}  
	  ?></div></td>
...
?> <font color="#666666"> <i> ( <? echo $is[jj]; ?> ) </i></font><?
		  ...
		  <td><div align="left"><? echo "$nn[TglKeluar]"; ?></div></td>
	 		  ...
	 		  <td><div align="left"><? $pala=strtoupper($nn[Opr]);
			  if($pala=='NUR'){
			  ?><font color="#003399"><B><?
			  echo $pala;
			  ?></B></font><? 
	  ...
	  $nomer++; 
	    ...
	    <td colspan="24" bgcolor="#5ABB52">&nbsp;</td>
	    </tr>
	  </table>
	  </div></td>
   </tr>
