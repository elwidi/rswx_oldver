...
<head><script type='text/javascript' src='js/jquery-1.3.2.min.js'></script>
  ...
  <script language="javascript">
    $(function(){
		...
		$('#suggest2').autocomplete('xx_00.php', 
			{
				...
				parse: function(data){ 
					...
					for (var i=0; i < data.length; i++) {
						parsed[i] = {
							data: data[i],
							value: data[i].Kota  // nama field yang dicari
...
str += '<u>Kelurahan '+data.Kelurahan+'</u><br />Kecamatan.'+data.Kecamatan+'<br />Kota: '+data.Kota+' <br>Jalan'+data.Jalan+'<br>Propinsi'+data.Propinsi;;					
					str += '';
					return str;
				},
...
var Result =''+data.id+'';;
location.href="casaa.php?bas=8&kadal=4&id=<? echo $id; ?>&IdKota=" + Result;
			}
		);
	});
  </script></head>
