...
<head><script type='text/javascript' src='js/jquery-1.3.2.min.js'></script>
  ...
  <script language="javascript">
    $(function(){
		...
		$('#suggest').autocomplete('loka_s.php', 
			{
				...
				parse: function(data){ 
					...
					for (var i=0; i < data.length; i++) {
						parsed[i] = {
							data: data[i],
							value: data[i].Jalan  // nama field yang dicari
...
str += '<u>'+data.Jalan+'</u><br /Kota.'+data.Kota+'<br />Propinsi: '+data.Propinsi;;					
					str += '';
					return str;
				},
...
var Result =''+data.id+'';;
location.href="puasa_9_bulan.php?hari=anak3&id=" + Result;
			}
		);
	});
  </script></head>
