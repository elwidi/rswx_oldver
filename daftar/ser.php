...
<head><script type='text/javascript' src='js/jquery-1.3.2.min.js'></script>
  ...
  <script language="javascript">
    $(function(){
		...
		$('#suggest2').autocomplete('search92x.php', 
			{
				...
				parse: function(data){ 
					...
					for (var i=0; i < data.length; i++) {
						parsed[i] = {
							data: data[i],
							value: data[i].ICD_CODE // nama field yang dicari
...
str += '<u>'+data.ICD_CODE+'</u><br />KODE ICD.'+data.DESCRIPTION;;					
					str += '';
					return str;
				},
...
var Result =''+data.id+'';;
location.href="7w.php?id=<? echo $id; ?>&IdDetil=" + Result;
			}
		);
	});
  </script> </head>
