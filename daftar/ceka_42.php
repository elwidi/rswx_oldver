...
<head><script type='text/javascript' src='js/jquery-1.3.2.min.js'></script>
  ...
  <script language="javascript">
    $(function(){
		...
		$('#suggest4').autocomplete('loka_s42.php', 
			{
				...
				parse: function(data){ 
					...
					for (var i=0; i < data.length; i++) {
						parsed[i] = {
							data: data[i],
							value: data[i].NamaKec  // nama field yang dicari
...
var Result =''+data.id+'';;
location.href="x2qaa.php?IdKel=<? echo $IdKel; ?>&IdKot=<? echo $IdKot; ?>&hari=supran&IdKec=" + Result;
			}
		);
	});
  </script></head>
