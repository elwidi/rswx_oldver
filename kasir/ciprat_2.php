...
<head><script type='text/javascript' src='js/jquery-1.3.2.min.js'></script>
  ...
  <script language="javascript">
    $(function(){
		...
		$('#suggest2').autocomplete('cari2.php', 
			{
				...
				parse: function(data){ 
					...
					for (var i=0; i < data.length; i++) {
						parsed[i] = {
							data: data[i],
							value: data[i].Alamat // nama field yang dicari
...
str += '<u>'+data.Alamat+'</u><br />No RM.'+data.NoPasien+'<br />NAMA PASIEN: '+data.NamaPasien+'<br> Telp'+' :'+data.Telp+'<br>TGL LAHIR : '+data.tgllhr;;					
					str += '';
					return str;
				},
...
var Result =''+data.id+'';;
location.href="x1_e.php?mas=hari&id=" + Result;
			}
		);
	});
  </script></head>
