...
include "../konek.php";
include "../ceke.php";
 $dfldkfd_Ffgff = mysql_query("SELECT * FROM oder_2 WHERE id='$id' AND Stat>=1 AND Stat<=2");
...
<tr>
<td>JENIS PERMINTAAN</td><td>:</td><td><div align="left"><?
if($ok[Stak]==1){
echo " RUTIN";
}else{
echo "NON  RUTIN";
...
<td width="128" >NO TXN</td>
<td width="3">:</td>
<td width="657"><div align="left">MT-<? echo  ($ok[id]); ?></div></td>
</tr>
<tr>
...
<TD>NAMA BARANG </TD>
<td>:</td><td><div align="left">
<input type="text" name="NamaObat" maxlength="50" size="20" class="style1888" <? if($ok[Stat]>=1){ ?> disabled="disabled"<? } ?>>
...
<tr>
  ...
  <input   onMouseOver="movein(this,'');window.status='';" onMouseOut="moveout(this);window.status='';"   type="submit" value="Submit" name="fgf" class="style1888"  <? if($ok[Stat]>=1){ ?> disabled="disabled"<? } ?>>
  ...
  'wCompare4f4d12', 'width=580,height=500,left=270,Top=150,resizable=yes,directories=no,status=no,toolbar=no,menubar=0,location=no,scrollbars=yes'); 
	...
	<input type="button"  <? if($ok[Stat]==2){ ?> disabled="disabled"<? } ?> name="fgfg" value="Terima Data"  onClick="wCompare4f23232324d12 = window.open('cw_1938n.php?id=<? echo $id; ?>', 
...
<tr valign="top">
  <td colspan="3"><div align="left">
   <iframe src="n6x2.php?id=<? echo $id; ?>" width="800" height="250" frameborder="0"></iframe>
  </div></td>
</tr>
</table>
</div>
